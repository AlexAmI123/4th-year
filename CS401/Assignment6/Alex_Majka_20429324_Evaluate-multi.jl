...
begin
		directory = "../outputs/20231219_1608/" #CHANGE HERE
...
# ╔═╡ b6a53b09-fe00-432c-8fea-c61041373dd4
md"""
# Conclusion

## Quick disclaimer, I was unable to run the training data after changing it as the training was taking over double the time it took to run it initially. Later on it wouldn't even get past the model creation stage without any changes that couls possibly break it. The Train_multi.jl/pdf is the same as the file that was used to generate the output for this evaluation

###
The reason why I was able to slightly improve the model's performance was because I added some transformerEncoderBlocks, one of which made use of Attention heads to improve the models capacity to capture complex patterns and to focus on different elements of the input.
I also added some dense layers.
I added dropout layers to prevent overfitting.

Apart from this I experimented with different values for the Dropout Rate to prevent overfitting aswell as the size of the embedding to find a sweet spot.
"""
