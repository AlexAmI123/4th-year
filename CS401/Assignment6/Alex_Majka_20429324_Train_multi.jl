...
begin
	path = normpath(joinpath(@__DIR__, "..", "assignment6/datasets", "amazon_reviews_multi", "en", "1.0.0"))
	...
	hyperparameters = Dict(
	    ...
	    "nlabels" => 5,
	    "pdrop" => 0.75, #CHANGE HERE FROM 0.1->0.5->0.75
	    "dim_embedding" => 128 #CHANGE HERE FROM 32->64->128
	)
...
begin
	...
	model = TransformersLite.TransformerClassifier(
	    ...
	    TransformerEncoderBlock[
	    	TransformerEncoderBlock(4, dim_embedding, dim_embedding * 4; 	pdrop=pdrop), #Added ,
			
			TransformerEncoderBlock(4, dim_embedding, dim_embedding * 4; 		pdrop=pdrop), #Added extra transformer block normal
			
			    TransformerEncoderBlock(4, dim_embedding, dim_embedding * 4, nheads=8; pdrop=pdrop), #Added extra encoder block with attention Head
	    ],
		Dropout(pdrop), #Adding Dropout Layers
	    Dense(dim_embedding, 1), 
	    FlattenLayer(),
	    Dense(max_length, nlabels),
		Dropout(pdrop), #Adding Dropout Layers
		Dense(dim_embedding, 256, relu), #Adding dense layers
		Dropout(pdrop), #Adding Dropout Layers
    	Dense(256, 128, relu), #Adding dense layers
	    )
	display(model)
	println("")
